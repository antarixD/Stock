...
def csv_to_dataset(csv_path):
    ...
    pdUtils.table(data.head(100), 'keys')
    data = data.drop(0, axis=0)

    #another way to convert dataframe to array of numpy
    data = data.values
    ...
    next_day_open_values_normalised = np.array([data_normalised[:, 2][i + history_points].copy() for i in range(len(data_normalised) - history_points)])
    next_day_open_values_normalised = np.expand_dims(next_day_open_values_normalised, -1)


    next_day_open_values = np.array([data[:, 2][i + history_points].copy() for i in range(len(data) - history_points)])
    ...
    for his in ohlcv_histories_normalised:
        # note since we are using his[1] we are taking the SMA of the closing price
        sma_of_Prev_last_close = np.mean(his[:, 0])
        sma_of_last_close = np.mean(his[:, 5])
        sma = np.mean(his[:, 2])
        macd = calc_ema(his, 12) - calc_ema(his, 26)
        technical_indicators.append(np.array([sma, macd, sma_of_Prev_last_close,sma_of_last_close]))

        #use below for better 1
