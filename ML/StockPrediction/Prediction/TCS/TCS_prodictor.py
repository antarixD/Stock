...
#from StockPrediction.Prediction.TCS.util_High import csv_to_dataset, history_points
#test_set_name = '/home/antarix/Desktop/naive bayes/Stocks/Data/CALSOFT.csv'
test_set_name = 'D:/naive bayes/Stocks/Data/CALSOFT.csv'
...
ohlcv_histories, technical_indicators, next_day_open_values, unscaled_y, y_normaliser = csv_to_dataset(test_set_name)
print("<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>??????????????????????????")
