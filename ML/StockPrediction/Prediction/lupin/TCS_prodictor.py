...
#from ML.StockPrediction.Prediction.TCS.util_Low import csv_to_dataset, history_points
from StockPrediction.Prediction.TCS.util_Low import csv_to_dataset, history_points

#test_set_name = '/home/antarix/Desktop/naive bayes/Stocks/Data/TCS.csv'
test_set_name = 'D:/naive bayes/Stocks/Data/TCS.csv'
...
ohlcv_histories, technical_indicators, next_day_open_values, unscaled_y, y_normaliser = csv_to_dataset('/home/antarix/Desktop/naive bayes/Stocks/Data/TCS.csv')
print("<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>??????????????????????????")
...
model.fit(x=[ohlcv_train, tech_ind_train], y=y_train, batch_size=32, epochs=100, shuffle=True, validation_split=0.1)
